zplug "junegunn/fzf-bin",\
    ...
    hook-load:"""
        ...
        """
        export FZF_DEFAULT_COMMAND='fd --hidden --type file --no-ignore --exclude "/.git/" --exclude "/dist"'
        # export FZF_DEFAULT_OPTS='--ansi --height 40% --reverse --no-border --multi'
        export FZF_DEFAULT_OPTS='--ansi --multi --height 80%'
        # export FZF_CTRL_T_COMMAND="$FZF_DEFAULT_COMMAND"
        export FZF_DEFAULT_COMMAND='fd --type file --hidden --follow --no-ignore --exclude .git --exclude node_modules --exclude dist'
        export FZF_CTRL_T_COMMAND="$FZF_DEFAULT_COMMAND"
zplug "junegunn/fzf",\
    as:command,\
    use:bin/fzf-tmux,\
